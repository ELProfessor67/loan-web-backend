...
export const updateState = catchAsyncError(async (req, res) => {
  ...
  let isAllAreVerfied = true;
  console.log('AAAA')
  let selectedVendor = []
  const list = ['mdse', 'freight', 'freight2', 'warehouse', 'misc'];

  list.map(type => {
    if (vendor[type].amount && vendor[type].status != "verified") {
      isAllAreVerfied = false;
    }
    if(vendor[type].amount){
      selectedVendor.push(type);
    }
  });

  console.log('creating merge document...',selectedVendor);
  if (isAllAreVerfied) {
    //creage merge sign document
    selectedVendor.map(async (type) => {
      let documentFirst = vendor[type].sign;
      documentFirst = path.join(process.cwd(), documentFirst);
      const documentSecond = path.join(process.cwd(), `/sign-document/${type}.png`);

      let images = []
      if (documentFirst.split('.').pop() === 'pdf') {
        
        images = [...images]
      } else {
        images = [...images,documentFirst]
      }

      
      if (documentSecond.split('.').pop() === 'pdf') {
        images = [...images]
      } else {
        images = [...images,documentSecond]
      }
